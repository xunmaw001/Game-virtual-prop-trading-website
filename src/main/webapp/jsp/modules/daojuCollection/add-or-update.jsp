    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑道具收藏</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">道具收藏管理</li>
                        <li class="breadcrumb-item active">编辑道具收藏</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">道具收藏信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group col-md-6 daojuCollectionTypesDiv">
                                        <label>类型</label>
                                        <select style="width: 450px" id="daojuCollectionTypesSelect" name="daojuCollectionTypes" class="form-control">
                                        </select>
                                    </div>
...
<script>
    ...
    // 下拉框数组
        <!-- 当前表的下拉框数组 -->
    var daojuCollectionTypesOptions = [];
        <!-- 级联表的下拉框数组 -->
    var daojuOptions = [];
    var yonghuOptions = [];
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("daojuCollection/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('adddaojuCollection');
    ...
    // 查询列表
        ...
        function daojuCollectionTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=daoju_collection_types", "GET", {}, (res) => {
                if(res.code == 0){
                    daojuCollectionTypesOptions = res.data.list;
                }
            });
        }
        <!-- 查询级联表的所有列表 -->
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationDaojucollectiontypesSelect(){
            var daojuCollectionTypesSelect = document.getElementById('daojuCollectionTypesSelect');
            if(daojuCollectionTypesSelect != null && daojuCollectionTypesOptions != null  && daojuCollectionTypesOptions.length > 0 ){
                for (var i = 0; i < daojuCollectionTypesOptions.length; i++) {
                    daojuCollectionTypesSelect.add(new Option(daojuCollectionTypesOptions[i].indexName,daojuCollectionTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(daojuCollectionTypesSelect != null && daojuCollectionTypesOptions != null  && daojuCollectionTypesOptions.length > 0 ) {
            for (var i = 0; i < daojuCollectionTypesOptions.length; i++) {
                if (daojuCollectionTypesOptions[i].codeIndex == ruleForm.daojuCollectionTypes) {//下拉框value对比,如果一致就赋值汉字
                        daojuCollectionTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                daojuId: "required",
                yonghuId: "required",
                daojuCollectionTypes: "required",
            },
            messages: {
                daojuId: "道具不能为空",
                yonghuId: "用户不能为空",
                daojuCollectionTypes: "类型不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var adddaojuCollection = window.sessionStorage.getItem("adddaojuCollection");
        if (adddaojuCollection != null && adddaojuCollection != "" && adddaojuCollection != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("daojuCollection/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationDaojucollectiontypesSelect();
            <!--  初始化级联表的下拉框  -->
