...
<div id="app">
    ...
    <ul class="cart_list_td clearfix" v-for="(item,index) in dataList" v-bind:key="index">
        <li class="col01"></li>
        <li class="col02"><img :src="item.daojuPhoto"></li>
        <li class="col03">{{item.daojuName}}<br><em>{{item.daojuNewMoney}} RMB</em></li>
    ...
    var vue = new Vue({
        ...
        methods: {
			...
			,addCartNum(index) {
				...
				layui.http.request(`daoju/info/${item.daojuId}`, 'get', {}, function(res) {
					// 库存限制
					if (res.data.daojuKucunNumber && res.data.daojuKucunNumber > 0 && res.data.daojuKucunNumber  <= item.buyNumber) {
						layui.layer.msg(`道具库存不足`, {
							time: 2000,
							icon: 5
						});
						return
			...
			// 购买
			buyClick() {
				localStorage.setItem('daojus', JSON.stringify(this.dataList));
				window.location.href = '../daojuOrder/confirm.jsp'
			}
        }
    });
