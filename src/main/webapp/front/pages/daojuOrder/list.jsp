...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的道具订单 </p>
    </div>
    <!-- 标题 -->
        ...
        <div class="modal-dialog" style="z-index: 3000">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <textarea id="daojuCommentbackContent" style="width: 500px;height: 300px"></textarea>
                </div>
                <%-- 模态框按钮 --%>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                    <button type="button" @click="submitDaojuCommentback()" class="btn btn-theme">评价</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="right-container" style="padding-top: 0;">
            <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
                <ul class="layui-tab-title">
                    <li class="layui-this" @click="pageList(0)">全部道具订单</li>
                    <li v-for="item in daojuOrderTypesList" :value="item.codeIndex"  @click="pageList(item.codeIndex)">{{item.indexName}}</li>
                </ul>
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.daojuName}}</td>
                            <td style="width: 250px;">
                                <img :src="item.daojuPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
                            <td style="width: 80px;">{{item.daojuValue}}</td>
                            <td style="width: 60px;">{{item.yonghuName}}</td>
                            <td style="width: 150px;">{{item.daojuOrderUuidNumber}}</td>
                            <td style="width: 150px;">{{item.buyNumber}}</td>
                            <td style="width: 150px;">{{item.daojuOrderTruePrice}}</td>
                            <td style="width: 150px;">{{item.daojuOrderCourierName}}</td>
                            <td style="width: 150px;">{{item.daojuOrderCourierNumber}}</td>
                            <td style="width: 80px;">{{item.daojuOrderValue}}</td>
                            <td style="width: 80px;">{{item.daojuOrderPaymentValue}}</td>
                            <td style="width: 150px;">{{item.insertTime}}</td>
                            <td style="width: 100px;">
                                <button v-if="item.daojuOrderTypes==1" @click="refund(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                ...
                                <button v-if="item.daojuOrderTypes==3" @click="receiving(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                ...
                                <button v-if="item.daojuOrderTypes==4" @click="commentback(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius btn-theme">
...
<script>
    var vue = new Vue({
        ...
        methods: {
            ...
            //列表查询
            ,pageList(daojuOrderTypes) {
                this.daojuOrderTypes = daojuOrderTypes;
                // 获取列表数据
                layui.http.request('daojuOrder/page', 'get', {
                    page: 1,
                    limit: this.limit,
                    daojuOrderTypes: (vue.daojuOrderTypes == 0)?null:vue.daojuOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('daojuOrder/page', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    daojuOrderTypes: (this.daojuOrderTypes == 0)?null:this.daojuOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`daojuOrder/pay?id=`+id, 'GET',{}, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
                            time: 2000,
                            icon: 6
            ...
            ,refund(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`daojuOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
            ...
            ,receiving(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`daojuOrder/receiving?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('成功收货', {
                            time: 2000,
                            icon: 6
            ...
            ,submitDaojuCommentback(){
                var daojuCommentbackContent = $("#daojuCommentbackContent").val();
                var id = $("#id").val();

                layui.http.request("daojuOrder/commentback?id="+id+"&commentbackText="+daojuCommentbackContent, 'get', {}, function (res) {
                    if(res.code==0){
                        layer.msg('评价成功', {
                            time: 2000,
                            icon: 6
    ...
    // 当前表的字典表
        ...
        function daojuOrderTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=daoju_order_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function daojuOrderPaymentTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=daoju_order_payment_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        // 获取列表数据
        http.request('daojuOrder/page', 'get',{
            page: 1,
            limit: limit,
            daojuOrderTypes: (vue.daojuOrderTypes == 0)?null:vue.daojuOrderTypes
        }, function(res) {
            ...
            laypage.render({
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('daojuOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            daojuOrderType: (vue.daojuOrderTypes == 0)?null:vue.daojuOrderTypes
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
