                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="daojuName" id="daojuName" placeholder="道具名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="daojuUuidNumber" id="daojuUuidNumber" placeholder="道具编号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="daojuTypes" id="daojuTypes" lay-filter="daojuTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in daojuTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                ...
                <div class="ng-inside-box ng-fault ng-container" aos="fade-up">
                    <div>
                        <div v-for="(item,index) in dataList" v-bind:key="index" class="col-md-3 col-sm-3 col-xs-6 w3gallery-grids">
                            <i @click="deleteDaojuCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                            <div class="imghvr-hinge-right figure" style="width: 100%" @click="jumpCheck('../daoju/detail.jsp?id='+item.daojuId , item.daojuDelete == null?'':item.daojuDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                <img :src="item.daojuPhoto?item.daojuPhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                <div class="agile-figcaption">
                                    <h4>{{item.daojuName}}</h4>
                                    <h4 v-if="item.daojuNewMoney">
                                        {{item.daojuNewMoney}} RMB
                                    </h4>
                                    <h4 v-else>
                                        {{item.daojuValue}}
                                    </h4>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            daojuCollectionTypesList : [],
        // 级联表字典表数据
            daojuTypesList : [],
            shangxiaTypesList : [],
            daojuCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteDaojuCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('daojuCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        // 查询当前字典表相关
            // 类型的查询和初始化
        daojuCollectionTypesSelect();
    // 当前表的字典表
        // 类型的查询
        function daojuCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=daoju_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.daojuCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function daojuTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=daoju_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.daojuTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#daojuName').val() != null && jquery('#daojuName').val() != "" && jquery('#daojuName').val() != "null" ) {
                param['daojuName'] = jquery('#daojuName').val();
            }
                     
            if (jquery('#daojuUuidNumber').val() != null && jquery('#daojuUuidNumber').val() != "" && jquery('#daojuUuidNumber').val() != "null" ) {
                param['daojuUuidNumber'] = jquery('#daojuUuidNumber').val();
            }
                                         
            var daojuTypes = $("#daojuTypes").find("option:selected").val();
            if (daojuTypes != null && daojuTypes != "" && daojuTypes != "null") {
                            param['daojuTypes'] = daojuTypes;
                        }
                                                                                                                                                                                    
            // 当前表
                                                
            //收藏只能查类型为1的
            param['daojuCollectionTypes'] = 1;

            // 获取列表数据
            http.request('daojuCollection/page', 'get', param, function(res) {
